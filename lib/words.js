...
export const getAllUnmemorizedWords = () => {
  const data = getWordbookData();
  const allUnmemorized = [];
  for (const wordbookName in data.wordbooks) {
    data.wordbooks[wordbookName].forEach(word => {
      if (!word.isMemorized) {
        allUnmemorized.push({ ...word, wordbookName }); // 단어장 이름도 함께 저장
      }
    });
  }
  return allUnmemorized;
};
